...
"""
  ...
  def normalHiveOverwriteGeneric = withEnvironment(path(getClass.getResource("/view-execution").toString)) {
    ...
    val genericHiveTable = new GenericPartitionedHiveTable[StringPair, String](
      "normalHive", "by_first", List("partition_first"), None
    )

    val s1 = TypedCsv[(String, String)]("partitioned-overwrite1").map { case (x, y) => (x, StringPair(x, y)) }
    val s2 = TypedCsv[(String, String)]("partitioned-overwrite2").map { case (x, y) => (x, StringPair(x, y)) }
    val exec = for {
      _ <- genericHiveTable.writeToSink(s1, false)
      - <- genericHiveTable.writeToSink(s2, false)
    } yield true
